...
$(document).ready(function() {
	 ...
	 */	
	function prepareUi() {
        $(".spanAccountSumm").html(Math.round(account_data.accountSumm * 100, 2) / 100 + "грн");
        $(".spanAccountOutSumm").html(Math.round(account_data.accountOutSumm * 100, 2) / 100 + "грн");
		...
		$("#tableAllAdvertise").jqGrid({
			...
			subGridRowExpanded: function(subgrid_id, row_id) {
				...
				$("#" + subgrid_table_id).jqGrid({
                    ...
                    'CTR', 'Показы в видимой части экрана','Среднее время до клика', 'Средняя цена', 'Расчетный доход'],
                    colModel: [
                      { name: 'Title', index: 'Title', width: 200, align: 'left', sortable: false, classes: 'subgrid_cell'},
                      { name: 'Impressions', index: 'Impressions', width: 80, align: 'center', sortable: false, classes: 'subgrid_cell'},
                      { name: 'Clicks', index: 'Clicks', width: 80, align: 'center', sortable: false, classes: 'subgrid_cell'},
                      { name: 'CTR', index: 'CTR', width: 90, align: 'center', sortable: false, classes: 'subgrid_cell'},
                      { name: 'ViewPort', index: 'ViewPort', width: 90, align: 'center', sortable: false, classes: 'subgrid_cell'},
                      { name: 'ViewSecond', index: 'ViewSecond', width: 90, align: 'center', sortable: false, classes: 'subgrid_cell'},
			...
			subGridRowExpanded: function(subgrid_id, row_id) {
				...
				$("#" + subgrid_table_id).jqGrid({
		            ...
		            colModel: [
		              ...
		              { name: 'GuidE', index: 'GuidE', width: 100, align: 'center', sortable: false, classes: 'subgrid_cell' },
		              { name: 'GuidR', index: 'GuidR', width: 100, align: 'center', sortable: false, classes: 'subgrid_cell' },
					  ],
					//pager: pager_id,
					//beforeSelectRow: function(rowid, e) {
					//	return false;
		...
		open: function(event, ui) {
			...
			$.getJSON('/private/lastMoneyOutDetails', function(data) {
				...
				switch (data.paymentType) {
					...
					case 'webmoney_r':
						$("#moneyOut_paymentType").val(data.paymentType);
						$("#moneyOut_webmoneyLogin_r").val(data.webmoneyLogin_r);
						$("#moneyOut_webmoneyAccountNumber_r").val(data.webmoneyAccount_r);
						$("#moneyOut_phone_r").val(data.phone_r);
						break;
					case 'webmoney_u':
						$("#moneyOut_paymentType").val(data.paymentType);
						$("#moneyOut_webmoneyLogin_u").val(data.webmoneyLogin_u);
						$("#moneyOut_webmoneyAccountNumber_u").val(data.webmoneyAccount_u);
						$("#moneyOut_phone_u").val(data.phone_u);
						break;
					case 'card':
						$("#moneyOut_paymentType").val(data.paymentType);
					...
					case 'card_pb_ua':
						$("#moneyOut_paymentType").val(data.paymentType);
						$("#moneyOut_cardType_pb_ua").val(data.cardType_pb_ua);
						$("#moneyOut_cardCurrency_pb_ua").val(data.cardCurrency_pb_ua);
						$("#moneyOut_cardNumber_pb_ua").val(data.cardNumber_pb_ua);
						$("#moneyOut_cardName_pb_ua").val(data.cardName_pb_ua);
						$("#moneyOut_cardMonth_pb_ua").val(data.expire_month_pb_ua);
						$("#moneyOut_cardYear_pb_ua").val(data.expire_year_pb_ua);
						$("#moneyOut_cardPhone_pb_ua").val(data.phone_pb_ua);
						break;
					case 'card_pb_us':
						$("#moneyOut_paymentType").val(data.paymentType);
						$("#moneyOut_cardType_pb_us").val(data.cardType_pb_us);
						$("#moneyOut_cardCurrency_pb_us").val(data.cardCurrency_pb_us);
						$("#moneyOut_cardNumber_pb_us").val(data.cardNumber_pb_us);
						$("#moneyOut_cardName_pb_us").val(data.cardName_pb_us);
						$("#moneyOut_cardMonth_pb_us").val(data.expire_month_pb_us);
						$("#moneyOut_cardYear_pb_us").val(data.expire_year_pb_us);
						$("#moneyOut_cardPhone_pb_us").val(data.phone_pb_us);
						break;
					default:
...
//							reloadTableExistingInformers();
                  ...
                  var message = 'Неизвестная ошибка удаления сайта. Попробуйте позже.'
